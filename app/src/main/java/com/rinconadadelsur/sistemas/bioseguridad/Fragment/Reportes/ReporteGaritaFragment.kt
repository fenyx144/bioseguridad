...
class ReporteGaritaFragment : Fragment() {
    private var bg: FragmentReporteGaritaBinding? = null
...
var conn: ConexionSQLiteHelper? = null
    ...
    private fun mostrarTotales(xFecha: String?, xCencos: String?) {
        val count = bg!!.tlRepGarita.getChildCount()
        for (i in 1..<count) {
            val child = bg!!.tlRepGarita.getChildAt(i)
            if (child is TableRow) (child as ViewGroup).removeAllViews()
        }
        db = conn!!.getReadableDatabase()
        try {
            sql =
                "SELECT " + dbEstructura.c_rgFecha + "," + dbEstructura.c_rgCencos + "," + dbEstructura.c_rgNomColaborador +
                        ...
                        "'  ORDER BY " + dbEstructura.c_rgFecha + "," + dbEstructura.c_rgTurno
